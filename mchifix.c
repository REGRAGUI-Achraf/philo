...
int check_death(t_philo *philo)
{
    ...
    if (time_since_meal > philo->data->time_to_die)
    {
        ...
        if (!philo->data->dead) // On affiche et on stoppe une seule fois
        {
            philo->data->dead = 1;
...
void think_routine(t_philo *philo)
{
	...
	ft_usleep_safe(philo, time_to_think);
}
...
void	*philosopher_routine(void *arg)
{
	...
	if (philo->data->nb_philo % 2 == 0)
		ft_usleep_safe(philo, (philo->id % 2) * 10);
	else
		ft_usleep_safe(philo, (philo->id % 2) * philo->data->time_to_eat);
	...
	while (!simulation_stopped(philo->data))
	{
		...
		usleep(1); // mini pause à chaque tour
